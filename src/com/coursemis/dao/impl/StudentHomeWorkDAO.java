...
public class StudentHomeWorkDAO extends BaseDAO implements IStudentHomeWorkDAO  {
	...
	public List<Studenthomework> getStudentHomeworkListBySMId(int smid){
		...
		try {
			...
			Query query = session.createQuery("from Studenthomework sh where sh.sourcemanage.smId = "+smid);
			list = query.list();
	...
	public Studenthomework getStudenthomework(int shid){
		...
		try {
			...
			Query query = session.createQuery("from Studenthomework  where shId = "+shid);
	...
	/*public List<Studenthomework> getStudenthomeworkListByStudentId(int sid){
		...
		//List<Course> list = null;
		List<Studenthomework> course = null;
		try {
			System.out.println("trying...getStudenthomework");
			Transaction tx = session.beginTransaction();
			//澶栭敭鏌ヨ
			
			Query query = session.createQuery("from Studenthomework sh  where sh.student.SId = "+sid);
			course=(List<Studenthomework>)query.list();
			//course = (Course)list.get(0);
			//query.executeUpdate();///?
			tx.commit();
			//log.debug("get TbUser instance successful");
	...
	*/
	...
	public boolean updateStudenthomework(Studenthomework sh) {
		...
		try {
			...
			System.out.println("updateStudenthomework TbNotice instance successful");
			return true;
		} catch (RuntimeException e){
